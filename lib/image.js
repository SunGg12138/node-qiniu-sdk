...
exports.faceGroup = function(options){
  if (!options) return Promise.reject('options is required');
  if (!op) return Promise.reject('options.op is required');
  if (!data) return Promise.reject('options.data is required');
  if (!options.sdk) return Promise.reject('options.sdk is required');

  // 获取请求参数
  let request_options;
  try {
    request_options = Extends.Image.faceGroupOp(op, data);
  } catch (error) {
