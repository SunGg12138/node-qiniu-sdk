...
exports.Image = {
  ...
  processingFops: function (url, options) {
    ...
    debug('Image processingFops: url: s%, options: S%', url, options);
...
function getImageFop(type, options) {
  switch (type) {
    ...
    case 'watermark':
      ...
      var fop = 'watermark/1/image/' + Auth.urlsafe_base64_encode(options.image);
  ...
  getAvthumbFops: function(type, options){
    switch (type) {
      case 'avthumb':
        ...
        if (options.wmImage) fop += '/wmImage/' + Auth.urlsafe_base64_encode(options.wmImage);
        ...
        if (options.wmText) fop += '/wmText/' + Auth.urlsafe_base64_encode(options.wmText);
        if (options.wmGravityText) fop += '/wmGravityText/' + options.wmGravityText;
        if (options.wmFont) fop += '/wmFont/' + Auth.urlsafe_base64_encode(options.wmFont);
        if (options.wmFontColor) fop += '/wmFontColor/' + Auth.urlsafe_base64_encode(options.wmFontColor);
      ...
      case 'avthumb/m3u8':
        ...
        if (options.domain) fop += '/domain/' + Auth.urlsafe_base64_encode(options.domain.replace(/^http(s)?:\/\//, ''));
        ...
        if (options.pattern) fop += '/pattern/' + Auth.urlsafe_base64_encode(options.pattern);
        return fop;
    }
  }
};