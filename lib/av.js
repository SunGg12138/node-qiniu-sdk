...
exports.review = function(options){
  ...
  request_options.headers.Authorization = Auth.qiniu_token.call(options.sdk, request_options);
...
exports.avsmart = function(options){
  ...
  // 忽略压缩视频体积时的错误，1为忽略，0为不忽略。如果没能压缩视频大小, 则忽略该错误并且返回原视频
  if (options.ignoreError) {
    fops += '/ignore-error/' + options.ignoreError
  }
  ...
  if (options.saveas) {
    ...
    fops += '|saveas/' + Auth.urlsafe_base64_encode(options.saveas);
  }
...
exports.segment = function(options){
  ...
  // 构建fops参数
  let fops = 'segment/' + options.format + '/pattern/' + Auth.urlsafe_base64_encode(options.pattern);
  if (options.segtime) {
...
exports.concat = function(options){
  ...
  options.urls.forEach(item => {
    fops += Auth.urlsafe_base64_encode(item);
  });
...
exports.vsample = function(options){
  if (!options) return Promise.reject('options is required');
  if (!options.format) return Promise.reject('options.format is required');
  if (typeof options.ss === 'undefined') return Promise.reject('options.ss is required');
  if (typeof options.t === 'undefined') return Promise.reject('options.t is required');
  if (!options.pfop) return Promise.reject('options.pfop is required');
  ...
  if (options.pattern) fops += '/pattern/' + Auth.urlsafe_base64_encode(options.pattern);
...
exports.adapt = function(options){
  ...
  if (options.multiPrefix) {
    fops += '/multiPrefix/' + options.multiPrefix.map(item => Auth.urlsafe_base64_encode(item)).join(',');
  }
  if (options.vb) fops += '/vb/' + options.vb;
  if (options.ab) fops += '/ab/' + options.ab;
...
exports.pm3u8 = function(options){
  ...
  if (!options.pipe && !options.path) return Promise.reject('options.pipe and options.path have at least one');
  
  let pipe = options.pipe || fs.createWriteStream(options.path);

  let M3U8DownloadURI = Auth.download_token.call(options.qiniu, options);
  ...
  return rp({ url: M3U8DownloadURI + '&' + fops, pipe: pipe });
};