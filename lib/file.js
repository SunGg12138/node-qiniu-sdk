...
File.prototype.upload = function(options){
  ...
  if (options.stream || options.path) {
    ...
    options.file = Buffer.from(options.txt, options.encoding);
  }
...
File.prototype.copy = function(dest, isForce){
  let destBucket = this.bucketName
  let destFileName = dest
  if(dest.split(':').length == 2){
    destBucket = dest.split(':')[0]
    destFileName = dest.split(':')[1]
  }
  let options = {
    _type: 'copy',
