...
describe('File 相关方法测试', function () {
  ...
  before(async function () {
    ...
    // 随机个名字
    CONST.bucketName_2 = new Date().getTime() + '';

    let result2 = await qiniu.bucket(CONST.bucketName_2).mk();
    debug('创建bucket：%s并返回：%s', CONST.bucketName_2, JSON.stringify(result2));
  ...
  it('tabZone 切换区域', async function () {
    // 来回切换一次区域
    let file = qiniu.file(CONST.bucketName + ':image.png');
    expect(file.zone === 'z0').to.be.ok;
    file.tabZone('z2');
    expect(file.zone === 'z2').to.be.ok;
  });
  it('upload 使用流上传', async function () {
    let result = await qiniu.file(CONST.bucketName + ':image.png')
  ...
  it('upload 上传文本', async function () {
    let result = await qiniu.file(CONST.bucketName + ':upload.txt.js')
  ...
  it('upload 直传接口', async function () {
    let result = await qiniu.file(CONST.scope).upload({ path: __filename })
    debug('上传文件并在储存空间的名字为：%s并返回：%s', CONST.fileName, JSON.stringify(result));
    expect(result).to.be.an('object');
    expect(result.error).to.be.undefined;
  });
  it('copy 资源复制', async function () {
    ...
    // 成功会返回''
    let result = await qiniu.file(CONST.scope).copy('f_copy.js');
  ...
  it('copy 资源复制，跨储存桶测试', async function () {
    // 官方文档：https://developer.qiniu.com/kodo/api/1254/copy
    // 成功会返回''
    let result = await qiniu.file(CONST.scope).copy(CONST.bucketName_2 + ':f_copy.js');
  ...
  it('move 资源移动／重命名', async function () {
    ...
    // 成功会返回''
    let result = await qiniu.file(CONST.bucketName + ':f_copy.js').move('f_copy_move.js');
  ...
  it('move 资源移动／重命名，跨储存桶测试', async function () {
    // 官方文档：https://developer.qiniu.com/kodo/api/1288/move
    // 成功会返回''
    let result = await qiniu.file(CONST.bucketName + ':f_copy_move.js').move(CONST.bucketName_2 + ':f_copy_move_2.js');
  ...
  it('chstatus 修改文件状态', async function () {
    ...
    // 成功会返回''
    // 禁用状态
    let r1 = await qiniu.file(CONST.scope).chstatus(1);
    debug('修改文件状态并返回：%s', JSON.stringify(r1));
    expect(r1 === '').to.be.ok
    // 启用状态
    let r2 = await qiniu.file(CONST.scope).chstatus(0);
  ...
  it('deleteAfterDays 更新文件生命周期', async function () {
    ...
    // 成功会返回''
    let result = await qiniu.file(CONST.scope).deleteAfterDays(7);
  ...
  it('chtype 修改文件存储类型', async function () {
    // 官方文档：https://developer.qiniu.com/kodo/api/4173/modify-the-file-status
    // 成功会返回''
    let result = await qiniu.file(CONST.scope).chtype(1);
    debug('修改文件存储类型并返回：%s', JSON.stringify(result));
    expect(result === '').to.be.ok
  });
  it('stat 资源元信息查询', async function () {
    let result = await qiniu.file(CONST.scope).stat();
  ...
  it('chgm 资源元信息修改', async function () {
    ...
    // 成功会返回''
    let result = await qiniu.file(CONST.scope).chgm('image/png');
    debug('资源元信息修改并返回：%s', JSON.stringify(result));
    expect(result === '').to.be.ok
  });
  it('fetch 第三方资源抓取', async function () {
    let result = await qiniu.file(CONST.bucketName + ':fetch.png').fetch('https://qiniu.staticfile.org/static/images/qiniu_logo.5249e634.png');
  ...
  it('prefetch 镜像资源更新', async function () {
    try {
      // 获取当前bucket的域名集合
      let CONST_bucket_domains = await qiniu.bucket(CONST.bucketName).domain();
      debug('当前bucket的域名集合为：%s', JSON.stringify(CONST_bucket_domains));
      ...
      let r2 = await bucket.image(CONST_bucket_domains[0]);
      debug('设置Bucket镜像源并返回：%s', JSON.stringify(r2));
      expect(r2).to.be.an('object');
      expect(r2.error).to.be.undefined;

      // 使用prefetch同步镜像文件
      let r3 = await qiniu.file(newBucketName + ':' + CONST.fileName).prefetch();
  ...
  it('delete 删除接口', async function () {
    let result = await qiniu.file(CONST.scope).delete();
  ...
  after(async function () {
    ...
    let result2 = await qiniu.bucket(CONST.bucketName_2).drop();
